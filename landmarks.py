...
while True:
    ...
    for k,d in enumerate(detections): #For each detected face  
        ...
        for i in range(1,68): #There are 68 landmark points on each face
            if i == 7:
                cv2.circle(landmark, (shape.part(i).x, shape.part(i).y), 2, (255,255,255), thickness=-1) #For each point, draw a red circle with thickness2 on the original frame
            else:    
